...
class index extends admin {
	...
	public function login() {
	    if(isset($_GET['dosubmit'])) {
			//不为口令卡验证
			if (!isset($_GET['card'])) {
				$username = isset($_POST['username']) ? trim($_POST['username']) : ajaxReturn(L('nameerror'),"","",'error');
				$code = isset($_POST['code']) && trim($_POST['code']) ? trim($_POST['code']) : ajaxReturn(L('input_code'),"","",'error');
				if ($_SESSION['code'] != strtolower($code)) {
					$_SESSION['code'] = '';
                    ajaxReturn(L('code_error'),"","",'error');
			...
			if(!is_username($username)){
                ...
                ajaxReturn(L('username_illegal'),"","",'error');
			}
			//密码错误剩余重试次数
			...
			if($rtime['times'] >= $maxloginfailedtimes) {
				...
				if($minute>0) ajaxReturn(L('wait_1_hour',array('minute'=>$minute)));
			}
			//查询帐号
			...
			if($r['password'] != $password) {
				...
				ajaxReturn(L('password_error',array('times'=>$times)),'?m=admin&c=index&a=login',3000,"error");
			}
			...
			ajaxReturn(L('login_success'),'?m=admin&c=index',"success");
			//同步登陆vms,先检查是否启用了vms
